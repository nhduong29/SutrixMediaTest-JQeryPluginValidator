...
;(function ( $, window, document, undefined ) {
	...
	"use strict";
		...
		$.extend(Plugin.prototype, {
				init: function () {
					//Parse data attribute object to nhduongValidator object
					this.dataAttributesParse();
					var that =this;
					...
					if(Object.keys(this.settings.fields).length>0){
						...
						if(Object.keys(this.settings.dataAttributes).length>0){
							that.numOfFiedValidate = Object.keys(that.settings.dataAttributes).length;
						}
					}
					...
					$(".form-control").change(function(){
						if(that.settings.fields[this.id]){
							...
							that.validatorsHandler(this.id,that.settings.dataAttributes[this.id].validators);
							that.stateSubmit();
						}
					});
				},
				 ...
				 * Parse data attribute object to nhduongValidator object
				 */
				dataAttributesParse : function(){
					for(var i=0;i<this.element.length;i++){
						var data = $(this.element[i]).data();
						if(Object.keys(data).length >0){
							var id = $(this.element[i]).attr("id");
							$.extend(this.settings.dataAttributes,JSON.parse('{"'+id+'":{"validators":'+JSON.stringify(this.convertDataAttributeToObj(data))+'}}'));
						}
					}
				},
				 ...
				 * @returns {{}}
				 */
				convertDataAttributeToObj : function(data){
					var validatorObj={};
					if(data.required){
						validatorObj = $.extend(validatorObj,JSON.parse('{"required":{"message":"'+data.requiredMessage+'"}}'));
					}
					if(data.regexp){
						validatorObj = $.extend(validatorObj,JSON.parse('{"regexp":{"regexp":"'+data.regexp+'"}}'));
						if(data.regexpMessage){
							$.extend(validatorObj.regexp,JSON.parse('{"message":"'+data.regexpMessage+'"}'));
						}
					}
					if(data.greaterthan){
						validatorObj = $.extend(validatorObj,JSON.parse('{"greaterThan":{"value":"'+data.greaterthan+'"}}'));
						if(data.greaterthanMessage){
							$.extend(validatorObj.greaterThan,JSON.parse('{"message":"'+data.greaterthanMessage+'"}'));
						}
					}
					if(data.identical){
						validatorObj = $.extend(validatorObj,JSON.parse('{"identical":{"withField":"'+data.identical+'"}}'));
						if(data.identicalMessage){
							$.extend(validatorObj.identical,JSON.parse('{"message":"'+data.identicalMessage+'"}'));
						}
					}
					if(data.maxlength){
						validatorObj = $.extend(validatorObj,JSON.parse('{"maxLength":{"max":"'+data.maxlength+'"}}'));
						if(data.maxlengthMessage){
							$.extend(validatorObj.maxLength,JSON.parse('{"message":"'+data.maxlengthMessage+'"}'));
						}
					}
					return validatorObj;
				},
				...
				validatorsHandler : function(element,validators){
					if(validators){
						...
						if(hasGreaterThanValidator){
							...
							if(!this.settings.passGreaterThan){
								return;
							}
						}
						...
						if(hasIdenticalValidator){
							...
							if(!this.settings.passIdentical){
								return;
							}
						}
						...
						if(hasMaxLengthValidator){
							...
							if(!this.settings.passMaxLength){
								return;
							}
						}
					}
